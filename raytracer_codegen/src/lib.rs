...
fn sphere_code(sphere: &Sphere) -> TokenStream {
    ...
    quote! {
        Arc::new(Sphere {
            ...
            radius: #size,
            mat_ptr: Arc::new(DiffuseLight::new(Vec3::new(#r, #g, #b), 1.0)),
        })
    }
}
...
pub fn make_spheres_impl(_item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let x_final: proc_macro2::TokenStream = tokens.into_iter().collect();

    let result = proc_macro::TokenStream::from(quote! {
        fn make_spheres() -> Vec<Arc<Sphere>> {
            let mut spheres = vec![];
            #x_final
            spheres
        }
...
// #[proc_macro]
// pub fn make_BVHNode(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
